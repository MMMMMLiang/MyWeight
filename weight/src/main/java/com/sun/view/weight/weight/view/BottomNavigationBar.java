package com.sun.view.weight.weight.view;
...
import com.sun.view.weight.R;
import com.sun.view.weight.animations.Techniques;
import com.sun.view.weight.animations.YoYo;
import com.sun.view.weight.utils.LogUtil;
import com.sun.view.weight.weight.adapter.NaViewPagerAdapter;
import com.sun.view.weight.weight.constant.NaAnim;
import com.sun.view.weight.weight.listener.OnAddClickListener;
...
public class BottomNavigationBar extends LinearLayout {
    ...
    private NaViewPagerAdapter adapter;
    ...
    private void initViews(Context context, AttributeSet attrs) {
        contentView = (RelativeLayout) View.inflate(context, R.layout.na_container_layout, null);
        addViewLayout = contentView.findViewById(R.id.na_add_view_ll);
        AddContainerLayout = contentView.findViewById(R.id.na_add_rl);
        empty_line = contentView.findViewById(R.id.na_empty_line);
        navigationLayout = contentView.findViewById(R.id.na_navigation_ll);
        mViewPager = contentView.findViewById(R.id.na_mViewPager);
        lineView = contentView.findViewById(R.id.na_common_horizontal_line);
        lineView.setTag(-100);
    ...
    private void toDp() {
        navigationHeight = NaUtil.dip2px(getContext(), navigationHeight);
        iconSize = NaUtil.dip2px(getContext(), iconSize);
        hintPointSize = NaUtil.dip2px(getContext(), hintPointSize);
        hintPointTop = NaUtil.dip2px(getContext(), hintPointTop);
        hintPointLeft = NaUtil.dip2px(getContext(), hintPointLeft);

        msgPointLeft = NaUtil.dip2px(getContext(), msgPointLeft);
        msgPointTop = NaUtil.dip2px(getContext(), msgPointTop);
        msgPointSize = NaUtil.dip2px(getContext(), msgPointSize);
        msgPointTextSize = NaUtil.sp2px(getContext(), msgPointTextSize);

        tabTextTop = NaUtil.dip2px(getContext(), tabTextTop);
        tabTextSize = NaUtil.sp2px(getContext(), tabTextSize);


        //Add
        addIconSize = NaUtil.dip2px(getContext(), addIconSize);
        addLayoutHeight = NaUtil.dip2px(getContext(), addLayoutHeight);
        addLayoutBottom = NaUtil.dip2px(getContext(), addLayoutBottom);
        addTextSize = NaUtil.sp2px(getContext(), addTextSize);
        addTextTopMargin = NaUtil.dip2px(getContext(), addTextTopMargin);
    }
    ...
    public void buildNavigation() {
        if ((titleItems.length != normalIconItems.length) || (titleItems.length != selectIconItems.length) || (normalIconItems.length != selectIconItems.length)) {
            LogUtil.e("EasyNavigation", "请传入相同数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }
        ...
        for (int i = 0; i < tabCount; i++) {
            View itemView = View.inflate(getContext(), R.layout.na_tab_layout, null);
            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.MATCH_PARENT);
            params.width = NaUtil.getScreenWidth((Activity) getContext()) / tabCount;

            itemView.setLayoutParams(params);
            itemView.setId(i);

            TextView text = itemView.findViewById(R.id.na_tab_text_tv);
            ImageView icon = itemView.findViewById(R.id.na_tab_icon_iv);
            ...
            View hintPoint = itemView.findViewById(R.id.na_red_point);
            ...
            //消息红点
            TextView msgPoint = itemView.findViewById(R.id.na_msg_point_tv);
            msgPoint.setTextSize(NaUtil.px2sp(getContext(), msgPointTextSize));
            ...
            text.setTextSize(NaUtil.px2sp(getContext(), tabTextSize));
    ...
    private void setViewPagerAdapter() {
        adapter = new NaViewPagerAdapter(fragmentManager, fragmentList);
    ...
    public void buildAddNavigation() {
        if (titleItems.length != normalIconItems.length || titleItems.length != selectIconItems.length) {
            LogUtil.e("Easynavigation", "请传入相同数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }
        tabCount = titleItems.length;
        if (tabCount % 2 == 0) {
            LogUtil.e("EasyNavigation", "MODE_ADD模式下请传入奇奇奇奇奇奇奇奇奇奇奇数数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }

        if (addAsFragment) {
            if (fragmentList.size() < tabCount) {
                LogUtil.e("EasyNavigation", "MODE_ADD模式下/addAsFragment=true时Fragment的数量应和传入tab集合数量相等");
                return;
            }
        } else {
            if (fragmentList.size() < tabCount - 1) {
                LogUtil.e("EasyNavigation", "MODE_ADD模式下/addAsFragment=false时Fragment的数量应比传入tab集合数量少一个");
                return;
            }
        }

        buildAddViewNavigationTwo();
    }

    private void buildAddViewNavigationTwo(){
        removeNavigationAllView();
        setViewPagerAdapter();
        for (int i = 0; i < tabCount; i++) {
            ...
            if (i == tabCount / 2) {
                ...
                RelativeLayout.LayoutParams addItemParams = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT,
                        RelativeLayout.LayoutParams.MATCH_PARENT);
                addItemParams.width = NaUtil.getScreenWidth((Activity) getContext()) / tabCount;
                ...
                final RelativeLayout.LayoutParams linearParams = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                ...
                addText.setTextSize(NaUtil.px2sp(getContext(), addTextSize));
                LayoutParams addTextParams = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                        ViewGroup.LayoutParams.WRAP_CONTENT);
                ...
                if (addLayoutRule == RULE_CENTER) {
                    ...
                    linearParams.addRule(RelativeLayout.ABOVE, R.id.na_empty_line);
                    if (addAlignBottom) {
    ...
    public void buildAddViewNavigation() {
        ...
        if (tabCount % 2 == 0) {
            LogUtil.e("EasyNavigation", "MODE_ADD_VIEW模式下请传入偶偶偶偶偶偶偶偶偶偶偶偶数数量的Tab文字集合、未选中图标集合、选中图标集合");
            return;
        }
        if (addAsFragment) {
            if (fragmentList.size() < tabCount) {
                LogUtil.e("EasyNavigation", "MODE_ADD_VIEW模式下/addAsFragment=true时Fragment的数量应比传入tab集合数量多一个");
                return;
            }
        } else {
            if (fragmentList.size() < tabCount - 1) {
                LogUtil.e("EasyNavigation", "MODE_ADD_VIEW模式下/addAsFragment=false时,Fragment的数量应和传入tab集合数量相等");
                return;
            }
        }
    ...
    public void setAddViewLayout(View addViewLayout) {
        FrameLayout.LayoutParams addParams = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
    ...
    public BottomNavigationBar addLayoutHeight(int addLayoutHeight) {
        this.addLayoutHeight = NaUtil.dip2px(getContext(), addLayoutHeight);
        return this;
    }

    public BottomNavigationBar scaleType(ImageView.ScaleType scaleType) {
        this.scaleType = scaleType;
        return this;
    }


    public BottomNavigationBar mode(int mode) {
        this.mode = mode;
        return this;
    }

    public BottomNavigationBar hasPadding(boolean hasPadding) {
        this.hasPadding = hasPadding;
        return this;
    }

    public BottomNavigationBar addIconSize(int addIconSize) {
        this.addIconSize = NaUtil.dip2px(getContext(), addIconSize);
        return this;
    }

    public BottomNavigationBar navigationBackground(int navigationBackground) {
        this.navigationBackground = navigationBackground;
        return this;
    }

    public BottomNavigationBar navigationHeight(int navigationHeight) {
        this.navigationHeight = NaUtil.dip2px(getContext(), navigationHeight);
        return this;
    }

    public BottomNavigationBar normalTextColor(int normalTextColor) {
        this.normalTextColor = normalTextColor;
        return this;
    }

    public BottomNavigationBar selectTextColor(int selectTextColor) {
        this.selectTextColor = selectTextColor;
        return this;
    }

    public BottomNavigationBar lineHeight(int lineHeight) {
        this.lineHeight = lineHeight;
        return this;
    }

    public BottomNavigationBar lineColor(int lineColor) {
        this.lineColor = lineColor;
        return this;
    }

    public BottomNavigationBar tabTextSize(int tabTextSize) {
        this.tabTextSize = NaUtil.sp2px(getContext(), tabTextSize);
        return this;
    }

    public BottomNavigationBar tabTextTop(int tabTextTop) {
        this.tabTextTop = NaUtil.dip2px(getContext(), tabTextTop);
        return this;
    }

    public BottomNavigationBar msgPointTextSize(int msgPointTextSize) {
        this.msgPointTextSize = NaUtil.sp2px(getContext(), msgPointTextSize);
        return this;
    }

    public BottomNavigationBar msgPointSize(int msgPointSize) {
        this.msgPointSize = NaUtil.dip2px(getContext(), msgPointSize);
        return this;
    }

    public BottomNavigationBar msgPointLeft(int msgPointLeft) {
        this.msgPointLeft = NaUtil.dip2px(getContext(), msgPointLeft);
        return this;
    }

    public BottomNavigationBar msgPointTop(int msgPointTop) {
        this.msgPointTop = NaUtil.dip2px(getContext(), msgPointTop);
        return this;
    }


    public BottomNavigationBar hintPointSize(int hintPointSize) {
        this.hintPointSize = NaUtil.dip2px(getContext(), hintPointSize);
        return this;
    }

    public BottomNavigationBar hintPointLeft(int hintPointLeft) {
        this.hintPointLeft = NaUtil.dip2px(getContext(), hintPointLeft);
        return this;
    }

    public BottomNavigationBar hintPointTop(int hintPointTop) {
        this.hintPointTop = NaUtil.dip2px(getContext(), hintPointTop);
        return this;
    }


    public BottomNavigationBar titleItems(String[] titleItems) {
        this.titleItems = titleItems;
        return this;
    }

    public BottomNavigationBar normalIconItems(int[] normalIconItems) {
        this.normalIconItems = normalIconItems;
        return this;
    }

    public BottomNavigationBar selectIconItems(int[] selectIconItems) {
        this.selectIconItems = selectIconItems;
        return this;
    }

    public BottomNavigationBar fragmentList(List<Fragment> fragmentList) {
        this.fragmentList = fragmentList;
        return this;
    }

    public BottomNavigationBar fragmentManager(FragmentManager fragmentManager) {
        this.fragmentManager = fragmentManager;
        return this;
    }

    public BottomNavigationBar anim(NaAnim anim) {
        if (anim != null) {
    ...
    public BottomNavigationBar addLayoutRule(int addLayoutRule) {
        this.addLayoutRule = addLayoutRule;
        return this;
    }

    public BottomNavigationBar canScroll(boolean canScroll) {
        this.canScroll = canScroll;
        return this;
    }

    public BottomNavigationBar smoothScroll(boolean smoothScroll) {
        this.smoothScroll = smoothScroll;
        return this;
    }


    public BottomNavigationBar onTabClickListener(OnTabClickListener onTabClickListener) {
        this.onTabClickListener = onTabClickListener;
        return this;
    }


    public BottomNavigationBar iconSize(int iconSize) {
        this.iconSize = NaUtil.dip2px(getContext(), iconSize);
        return this;
    }

    public BottomNavigationBar addLayoutBottom(int addLayoutBottom) {
        this.addLayoutBottom = NaUtil.dip2px(getContext(), addLayoutBottom);
        return this;
    }

    public BottomNavigationBar addAsFragment(boolean addAsFragment) {
        this.addAsFragment = addAsFragment;
        return this;
    }

    public BottomNavigationBar addCustomView(View customAddView) {
        this.customAddView = customAddView;
        return this;
    }

    public BottomNavigationBar addTextSize(int addTextSize) {
        this.addTextSize = NaUtil.sp2px(getContext(), addTextSize);
        return this;
    }

    public BottomNavigationBar addNormalTextColor(int addNormalTextColor) {
        this.addNormalTextColor = addNormalTextColor;
        return this;
    }

    public BottomNavigationBar addSelectTextColor(int addSelectTextColor) {
        this.addSelectTextColor = addSelectTextColor;
        return this;
    }

    public BottomNavigationBar addTextTopMargin(int addTextTopMargin) {
        this.addTextTopMargin = NaUtil.dip2px(getContext(), addTextTopMargin);
        return this;
    }

    public BottomNavigationBar addAlignBottom(boolean addAlignBottom) {
    ...
    public NaViewPagerAdapter getAdapter() {
        return adapter;
    }
